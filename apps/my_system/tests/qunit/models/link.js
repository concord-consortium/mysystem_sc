...
module("MySystem.Link", {
  setup: function () {
    ...
    node1   = store.createRecord(MySystem.Node, { 'guid': MySystem.Node.newGuid(), 'title': 'Test node 1', 'image': 'http://ccmysystem.appspot.com/images/At-Concord-Fall/lightbulb_tn.png', 'transformer': false });
    node2   = store.createRecord(MySystem.Node, { 'guid': MySystem.Node.newGuid(), 'title': 'Test node 2', 'image': 'http://ccmysystem.appspot.com/images/At-Concord-Fall/lightbulb_tn.png', 'transformer': false });
    store.commitRecords();
  },
...
test("Links should return the nodes they're linked to", function() {
  expect(2);
  var newLink = store.createRecord(MySystem.Link, {'guid': MySystem.Link.newGuid(), 'text': 'Test link for node returning' });
...
test("Link color should be editable and persistent in the store", function() {
  ...
  var link1 = store.createRecord(MySystem.Link, linkHash, linkHash.guid);
