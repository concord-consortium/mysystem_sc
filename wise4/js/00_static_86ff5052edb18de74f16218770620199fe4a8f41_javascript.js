...
}if(SC.BUNDLE_INFO[a]){return}SC.BUNDLE_INFO[a]={requires:[],styles:["css/00_static_0934be487cad8eacd9ac3b1a9dfaec501ec1db97_stylesheet-packed.css","css/00_static_0934be487cad8eacd9ac3b1a9dfaec501ec1db97_stylesheet.css"],scripts:["js/00_static_0934be487cad8eacd9ac3b1a9dfaec501ec1db97_javascript-packed.js"]}
})();MySystem=SC.Application.create({NAMESPACE:"MySystem",VERSION:"0.1.0",store:SC.Store.create().from(SC.Record.fixtures),canvasView:null,NOVICE_STUDENT:"novice",ADVANCED_STUDENT:"advanced"});
MySystem.nodePaletteController=SC.ArrayController.create({});MySystem.nodesController=SC.ArrayController.create(SC.CollectionViewDelegate,{selectedLinksBinding:"MySystem.mainPage.mainPane.topView.bottomRightView.bottomRightView.selectedLinks",allSelected:function(){var a=this.get("selectedLinks");
var b=this.get("selection").clone();b=b.addObjects(a.map(function(c){return c.get("model")
}));return b}.property("selectedLinks","selectedLinks.[]","selection").cacheable(),unselectAll:function(){var b=MySystem.mainPage.getPath("mainPane.topView.bottomRightView.bottomRightView");
if(b.get("selectedLinks")){b.set("selectedLinks",[]);b.linksDidChange()}var a=this.get("selection").clone();
this.deselectObjects(a)},selectFirstTransformation:function(b){this.unselectAll();
var a=b.firstUnannotatedTransformation();if(a){MySystem.nodesController.selectObject(b);
MySystem.nodesController.selectObjects(a.get("inLinks"),YES);MySystem.nodesController.selectObjects(a.get("outLinks"),YES);
this.promptForTransformationAnnotation(a)}},collectionViewDeleteContent:function(a,b,c){var d=c.map(function(e){return this.objectAt(e)
...
}else{return b}},addNode:function(f,d,e,c){var b;var a=MySystem.Node.newGuid();b=MySystem.store.createRecord(MySystem.Node,{title:f,image:d,position:{x:e,y:c},guid:a},a);
return YES},propertyWindowSelection:function(){var b=MySystem.nodesController.get("allSelected");
var a=MySystem.getPath("mainPage.propertyViewPane");var c=MySystem.getPath("mainPage.sentenceLinkPane");
if(b.length()===0){if(a.isPaneAttached){a.remove()}a.set("objectToEdit",null)}else{if(b.length()==1){if(!a.isPaneAttached&&!c.isPaneAttached){a.append();
a.becomeFirstResponder()}a.set("objectToEdit",b.firstObject())}else{if(!a.isPaneAttached&&!c.isPaneAttached){a.append()
}a.set("objectToEdit",null)}}}.observes("allSelected")});MySystem.storyController=SC.ObjectController.create({});
MySystem.storySentenceController=SC.ArrayController.create(SC.CollectionViewDelegate,{allowsMultipleSelection:NO,collectionViewDeleteContent:function(a,e,d){var c=d.map(function(f){return this.objectAt(f)
...
},addStorySentenceNoEdit:function(){var a;this.incrementOrderValues(0);a=MySystem.store.createRecord(MySystem.StorySentence,{guid:MySystem.StorySentence.newGuid(),order:0,bodyText:"Describe a new step in the story."});
return a},addLinksAndNodesToSentence:function(a,b){a.forEach(function(e,d,c){if(e.instanceOf(MySystem.Link)){b.get("links").pushObject(e)
}else{if(e.instanceOf(MySystem.Node)){b.get("nodes").pushObject(e)}else{SC.Logger.log("Bad item type "+e)
}}})},addDiagramConnectPane:function(c){var b=MySystem.getPath("mainPage.sentenceLinkPane");
var a=MySystem.mainPage.getPath("mainPane.topView.bottomRightView.bottomRightView");
MySystem.nodesController.unselectAll();a.selectObjects([]);if(!b.isPaneAttached){b.append();
b.becomeFirstResponder()}a.selectObjects(c.get("links"),true);a.get("classNames").push("sentence-linking");
MySystem.nodesController.selectObjects(c.get("nodes"),true);b.set("activeSentence",c)
},closeDiagramConnectPane:function(){var b=MySystem.getPath("mainPage.sentenceLinkPane");
var a=MySystem.mainPage.getPath("mainPane.topView.bottomRightView.bottomRightView");
var c=b.get("selectedObjects");var d=b.get("activeSentence");if(d){d.get("nodes").removeObjects(d.get("nodes"));
d.get("links").removeObjects(d.get("links"));this.addLinksAndNodesToSentence(c,d)
}if(b.isPaneAttached){b.remove()}a.get("classNames").pop();MySystem.nodesController.unselectAll();
b.set("activeSentence",null);MySystem.get("nodesController").get("content").forEach(function(e){e.notifyPropertyChange("transformationIcon")
})},turnOffOtherButtons:function(c){var a=MySystem.mainPage.getPath("mainPane.topView.bottomRightView.topLeftView.bottomRightView.sentencesView");
var b=a.get("contentView").get("childViews");b.forEach(function(d){if(d.get("linkButton")!=c){d.get("linkButton").set("value",NO)
}})},doneButtonPushed:function(){this.closeDiagramConnectPane();this.turnOffOtherButtons(null)
},createSentence:function(a){var b=this.addStorySentence();this.addLinksAndNodesToSentence([a],b);
this.addLinksAndNodesToSentence(a.get("links").map(function(c){return c.model}),b)
},incrementOrderValues:function(a){this.content.forEach(function(d,c,b){if(d.get("order")>=a){d.set("order",d.get("order")+1)
...
})}});MySystem.CouchDataSource=SC.DataSource.extend({_ids:{},_revs:{},fetch:function(a,b){this.log("CouchDataSource.fetch()");
...
}else{this.log=console.log}}}});MySystem.Activity=SC.Record.extend({paletteItems:SC.Record.toMany("MySystem.PaletteItem"),assignmentText:SC.Record.attr(String),energyTypes:SC.Record.toMany("MySystem.EnergyType")});
sc_require("models/activity");MySystem.Activity.FIXTURES=[{guid:"assign1",paletteItems:["pi1","pi2","pi3"],assignmentText:"<p>Make a MySystem to explain to Gwen how energy from the sun and the things people do BOTH contribute to global climate.</p><ul><li>Where does energy come from?</li><li>How does energy move?</li><li>Where does energy go?</li><li>How does energy change?</li></ul>",energyTypes:["et1","et2","et3","et4","et5"]}];
...
};sc_require("models/energy_flow");MySystem.EnergyFlow.FIXTURES=[];MySystem.EnergyType=SC.Record.extend({label:SC.Record.attr(String,{isRequired:YES,defaultValue:"Energy Flow"}),color:SC.Record.attr(String,{isRequired:YES}),isEnabled:SC.Record.attr(Boolean,{isRequired:YES,defaultValue:YES})});
...
a.linkStyle=this.get("linkStyle");a.model=this;return SC.Object.create(LinkIt.Link,a)
},_textChanged:function(){this.invokeOnce(this._setLabel);if(this.get("startNode")){this.get("startNode").notifyPropertyChange("links")
}if(this.get("endNode")){this.get("endNode").notifyPropertyChange("links")}}.observes(".text"),_colorChanged:function(){this.invokeOnce(this._setLinkStyle);
...
this.set("linkStyle",a)}});MySystem.Link.GuidCounter=100;MySystem.Link.newGuid=function(){return"link"+MySystem.Link.GuidCounter++
...
b.label=SC.clone(a.get("label"));return b};sc_require("models/link");MySystem.Link.FIXTURES=[{guid:"link1",text:"First link",startNode:"1",startTerminal:"a",endNode:"2",endTerminal:"b",color:"red"},{guid:"link2",text:"Second link",startNode:"1",startTerminal:"a",endNode:"3",endTerminal:"b",color:"red"}];
MySystem.Node=SC.Record.extend(LinkIt.Node,{image:SC.Record.attr(String),title:SC.Record.attr(String),transformer:SC.Record.attr(Boolean,{defaultValue:true}),toolTip:SC.Record.attr(String,{defaultValue:null}),outLinks:SC.Record.toMany("MySystem.Link",{inverse:"startNode",isMaster:YES}),inLinks:SC.Record.toMany("MySystem.Link",{inverse:"endNode",isMaster:YES}),sentences:SC.Record.toMany("MySystem.StorySentence",{inverse:"nodes",isMaster:NO}),transformations:SC.Record.toMany("MySystem.Transformation",{inverse:"node",isMaster:YES}),terminals:["a","b"],formFields:[Forms.FormView.row(SC.TextFieldView,{fieldKey:"image",fieldLabel:"Image:"}),Forms.FormView.row(SC.TextFieldView,{fieldKey:"title",fieldLabel:"Title:"})],inColorMap:[],outColorMap:[],links:function(){var e=[],d;
...
return"images/00_static_transformationNeededIcon.gif"
}else{this.set("toolTip",null);return"images/00_static_noTransformationNeededIcon.gif"
...
};sc_require("models/node");MySystem.Node.FIXTURES=[{guid:"1",image:"images/00_static_clay_red_tn.png",title:"A Node",transformer:false,inLinks:[],outLinks:["link1","link2"],position:{x:160,y:160},transformations:["trans1"]},{guid:"2",image:"images/00_static_hand_tn.png",title:"B Node",transformer:false,inLinks:["link1"],outLinks:[],position:{x:310,y:10}},{guid:"3",image:"images/00_static_lightbulb_tn.png",title:"Third Node",transformer:false,inLinks:["link2"],outLinks:[],position:{x:10,y:10}}];
MySystem.PaletteItem=SC.Record.extend({image:SC.Record.attr(String),title:SC.Record.attr(String)});
sc_require("models/palette_item");MySystem.PaletteItem.FIXTURES=[{guid:"pi1",title:"Clay",image:"images/00_static_clay_red_tn.png"},{guid:"pi2",title:"Hand",image:"images/00_static_hand_tn.png"},{guid:"pi3",title:"Bulb",image:"images/00_static_lightbulb_tn.png"}];
...
sc_require("models/story");MySystem.Story.FIXTURES=[{guid:1,storyHtml:"<p>Make a MySystem to explain to Gwen how energy from the sun and the things people do BOTH contribute to global climate.</p><ul><li>Where does energy come from?</li><li>How does energy move?</li><li>Where does energy go?</li><li>How does energy change?</li></ul>"}];
...
},mouseDown:function(b){var a=this;var c=MySystem.getPath("mainPage.mainPane.childViews").objectAt(0).getPath("bottomRightView.bottomRightView");
SC.Drag.addDropTarget(c);var d={event:b,source:a.get("parentView"),dragView:a,ghost:NO,slideBack:NO,data:{title:this.get("title")||"title",image:this.get("image")||"image"}};
SC.Drag.start(d)},mouseUp:function(a){arguments.callee.base.apply(this,arguments);
...
if(this.get("isSelected")){a.addClass("selected")}},icon:SC.ImageView.design({classNames:"image",useImageCache:true,layout:{top:20,width:68,height:68,centerX:0},value:"images/00_static_badge.png"}),mouseDown:function(d){var e=MySystem.getPath("mainPage.mainPane.childViews").objectAt(0).getPath("bottomRightView.bottomRightView");
...
},performDragOperation:function(g,i){var c=g.data.image;var j=g.data.title;var f=g.location.x;
var a=g.location.y;var h=this.parentView.get("frame").x;var e=this.get("frame").y;
var d=g.ghostOffset.x;var b=g.ghostOffset.y;MySystem.nodesController.addNode(j,c,f-h-d,a-e-b);
MySystem.nodesController.deselectObjects(MySystem.nodesController.get("allSelected"));
return SC.DRAG_COPY},didCreateLayer:function(){var c=this.get("frame");var b=this.$("canvas.base-layer");
if(b){b.attr("width",c.width);b.attr("height",c.height);if(b.length>0){var a=b[0].getContext("2d");
if(a){a.clearRect(0,0,c.width,c.height);this._drawLinks(a)}else{SC.Logger.warn("MySystem.CanvasView.render(): Canvas object context is not accessible.")
...
sc_require("core");MySystem.PropertyEditorPane=SC.PalettePane.extend({acceptsFirstResponder:YES,layout:{top:150,right:5,width:250,height:240},classNames:"property-editor".w(),displayProperties:"objectToEdit isSelected".w(),objectToEdit:null,isSelected:false,childViews:"".w(),propertiesForm:Forms.FormView.create({fields:"".w(),findField:function(b){var a=null;
...
var c=this.get("objectToEdit");if(c===null){if(f.parentView!==null){f.set("fields",[]);
...
this.get("objectToEdit").set(b,d)},keyDown:function(a){return this.interpretKeyEvents(a)?YES:NO
},deleteBackward:function(){MySystem.canvasView.deleteSelection();return YES},deleteForward:function(){MySystem.canvasView.deleteSelection();
return YES}});MySystem.SentenceView=SC.View.extend({childViews:"sentenceText linkButton".w(),classNames:"story-sentence",sentenceText:SC.LabelView.design({layout:{left:5,right:50},valueBinding:".parentView*content.bodyText",canEditContent:YES,canDeleteContent:YES,isEditable:YES}),linkButton:SC.ButtonView.design({layout:{right:5,width:40},titleMinWidth:45,value:NO,contentBinding:".parentView*content",buttonBehavior:SC.TOGGLE_BEHAVIOR,icon:"images/00_static_icon_link.gif",toolTip:"Link this sentence with part of the diagram",pushButton:function(){if(this.value){MySystem.storySentenceController.turnOffOtherButtons(this);
MySystem.storySentenceController.addDiagramConnectPane(this.content)}}.observes("value"),unPushButton:function(){if(!this.value){MySystem.storySentenceController.closeDiagramConnectPane()
}}.observes("value")})});MySystem.SentenceConnectPane=SC.PalettePane.design({layout:{top:150,right:5,width:150,height:150},classNames:"sentence-connect-pane".w(),activeSentence:null,selectedObjectsBinding:"MySystem.nodesController.allSelected",contentView:SC.View.design({childViews:"labelView doneButton".w(),labelView:SC.LabelView.design({layout:{left:5,right:5,top:5,width:140,height:80},value:"Select the nodes and links your sentence describes, then click the 'Done' button.",canEditContent:YES,canDeleteContent:YES,isEditable:YES,isEnabled:YES}),doneButton:SC.ButtonView.design({acceptsFirstResponder:YES,buttonBehavior:SC.PUSH_BEHAVIOR,layout:{left:10,right:10,bottom:10,height:20},title:"Done",toolTip:"Click to link and close this window",target:"MySystem.storySentenceController",action:"doneButtonPushed",theme:"capsule",isEnabled:YES})})});
sc_require("views/sentence");MySystem.UserStoryView=SC.View.extend({childViews:"toolbar sentencesView".w(),toolbar:SC.ToolbarView.design({layout:{top:0,left:0,right:0,height:30},childViews:"addButton".w(),anchorLocation:SC.ANCHOR_TOP,addButton:SC.ButtonView.design({layout:{centerY:0,height:20,width:150,left:10},title:"Add sentence to story",target:"MySystem.storySentenceController",action:"addStorySentence",toolTip:"Add a sentence to the story"})}),sentencesView:SC.ScrollView.design({hasHorizontalScroller:NO,layout:{top:30,bottom:0,left:0,right:0},backgroundColor:"white",contentView:SC.ListView.design({contentBinding:"MySystem.storySentenceController.arrangedObjects",selectionBinding:"MySystem.storySentenceController.selection",contentValueKey:"bodyText",rowHeight:20,exampleView:MySystem.SentenceView,canEditContent:YES,canDeleteContent:YES,canReorderContent:YES})})});
sc_require("views/user_story");MySystem.StoriesView=SC.SplitView.extend({layoutDirection:SC.LAYOUT_HORIZONTAL,topLeftView:SC.LabelView.design({layout:{top:0,right:0,left:0},anchorLocation:SC.ANCHOR_TOP,textAlign:SC.ALIGN_LEFT,backgroundColor:"#CCCCFF",tagName:"div",escapeHTML:NO,valueBinding:"MySystem.storyController.content",canCollapse:YES}),dividerView:SC.SplitDividerView,bottomRightView:MySystem.UserStoryView});
...
sc_require("views/sentence");sc_require("views/sentence_connect_pane");MySystem.mainPage=SC.Page.design({mainPane:SC.MainPane.design({childViews:"topView".w(),topView:SC.SplitView.design({defaultThickness:140,topLeftView:MySystem.NodePaletteView.design({layout:{top:0,bottom:0,left:15}}),dividerView:SC.SplitDividerView,bottomRightView:SC.SplitView.design({defaultThickness:130,layoutDirection:SC.LAYOUT_VERTICAL,topLeftView:MySystem.StoriesView,dividerView:SC.SplitDividerView,bottomRightView:MySystem.CanvasView.design({layout:{top:120,left:0,right:0,bottom:0},contentBinding:SC.Binding.from("MySystem.nodesController"),selectionBinding:"MySystem.nodesController.selection",linkSelectionBinding:"MySystem.nodesController.linkSelection",exampleView:MySystem.NodeView})})})}),propertyViewPane:MySystem.PropertyEditorPane.design({}),sentenceLinkPane:MySystem.SentenceConnectPane.design({}),transformationBuilderPane:MySystem.TransformationBuilderPane.design({}),transformationAnnotaterPane:MySystem.TransformationAnnotationPane.design({})});
sc_require("lib/old_format_json_parser");MySystem.studentMode=MySystem.ADVANCED_STUDENT;
MySystem.main=function main(){MySystem.getPath("mainPage.mainPane").append();SC.ExceptionHandler=MySystem.ExceptionHandler;
var b=MySystem.store.find(MySystem.Node);MySystem.nodesController.set("content",b);
var e=MySystem.store.find(MySystem.Activity,"assign1");MySystem.storyController.set("content",e.get("assignmentText"));
MySystem.nodePaletteController.set("content",e.get("paletteItems"));MySystem.energyTypes=[];
e.get("energyTypes").forEach(function(f){MySystem.energyTypes.push({label:f.get("label"),color:f.get("color"),isEnabled:f.get("isEnabled")})
...
};function main(){MySystem.main()}MySystem.clearCanvas=function(){var c;var b=MySystem.store.find(MySystem.Node);
for(c=0;c<b.get("length");++c){b.objectAt(c).destroy()}var a=MySystem.store.find(MySystem.Link);
for(c=0;c<a.get("length");++c){a.objectAt(c).destroy()}};MySystem.loadCanvas=function(){MySystem.clearCanvas();
var b=MySystem.store.find(MySystem.StudentState).objectAt(0);var a=JSON.parse(b.get("content"));
MySystem.parseOldFormatJson(a)};